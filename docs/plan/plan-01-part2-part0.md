...
| **P5** | `vocab/core/prefixes.yaml`, the bound **identities** in ADR-004's generated worklist, external graphs cached | the binding surface — **external identity only** (PA25) | P20 | — | **required** | Blocks P6a, P7, P10. **Clauses 2 and 3 are discharged, 2026-08-04**, by `vocab/external/`: 21 graphs cached, every bound term content-verified by fetch-and-grep, and each term's definition read out of the cached graph — `vocab/external/manifest.md`, `vocab/external/bound-terms.md`. **Clause 1's artifact now EXISTS — `vocab/core/prefixes.yaml`, written 2026-08-06, 24 prefixes, loaded by `SchemaView`.** This field read *which does not exist* for one commit after the file was created, because the same pass restated the **path** and left the **status** — the retraction discipline applied to one half of a sentence. **The clause is delivered and NOT verified, and this sentence has been restated once because its first version was wrong in H's own favour (B3).** It read *unfalsifiable until a file uses a CURIE, which is P6a* — implying P6a closes it. **P6a closes half.** `declared-prefix` guards the DECLARATION half: a used prefix must appear in the map, and until a file uses one it walks `classes`, `slots` and `enums` over a schema with none of the three, so cutting the map from 24 entries to one leaves `make lint` green. **Nothing guards whether a declared URI is the RIGHT one, at P6a or after** — `sosa: http://www.w3.org/ns/sosa-TYPO/` with a CURIE using it passes all nine rules at exit 0, because `jurisdiction` is satisfied by the host still being `www.w3.org`. **So P5's delivery rests on judgement permanently.** The check that would close it is specified at the foot of `vocab/core/prefixes.yaml` — compare each declared namespace against the corresponding sidecar's `namespace:` field, all 17 verified that way by hand and agreeing — and is deliberately not built: the sidecars are gitignored, so it needs a tracked namespace list, which is a third generated file and F19's shape again. Build it when something authored is wrong. **Path restated 2026-08-06, and it is a measurement:** this read `vocab/prefixes.yaml`, which is scanned by **nothing** — `drift-lint.py` targets `vocab/core/` and C4's grep targets core plus profiles. `vocab/core/` is scanned by all nine rules and falls inside invariant 2's scope, and a prefix map no instrument inspects is what C18 has recorded. **Size restated `L` → `S` by P20, 2026-08-05, and it is a measurement rather than a re-estimate:** two of three clauses are discharged artifacts on disk, so the remaining work is one file. **P5 is no longer the long pole**; PA5, PA17 and PA28's sizing claims are restated in the same pass |
...
restatement precedes the work. P5 is now wave 2, and **P9 is the only `L`** — so P5 is no longer the widest item either. Everything in wave 3 still waits on it, which is the part that survives.

> **`It is still the widest item` was false and the guard caught it.** Restated 2026-08-05 under B3. P5's size went `L` → `S` when its clauses 2 and 3 were discharged; the sizing sentence was not restated with it, and the phrase was **invisible to the guard because it wraps** — `widest` ends one line and `item` begins the next. This is the first live wrong claim the guard has found rather than a residue it was pointed at.
...
| **P5** | `vocab/core/prefixes.yaml` resolves every prefix used; **every external term in ADR-004's generated worklist** is content-verified by fetch-and-grep; external graphs are cached locally. **The local slots are NOT declared here** — clause 3 removed, see PA25. **Restated by P20, 2026-08-05.** This read *23 external terms* and *the ten local terms*, the population ADR-004 retires as unrecoverable. The worklist is the replacement and **no count is stated here** — a new total would be one more figure agreeing with an earlier one by accident. **Clauses 2 and 3 are discharged** (see notes); what this criterion still asks for is clause 1. **Form, added 2026-08-06 because this criterion said nothing about it:** the file is a **LinkML schema**, not a bare `prefixes:` map — it carries `id:` and `default_prefix: ohim`, without which `rule_jurisdiction` fails it, and whose remedy message names `vocab/profiles/`, which for this project's own namespace would be wrong | S |
