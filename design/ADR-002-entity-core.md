...
| `Place` | Zones, jurisdictions, facilities, sampling locations | ISO 19112 — **amended 2026-08-04 by ADR-006 Decision B.** This read `sosa:FeatureOfInterest, ISO 19112`; `FeatureOfInterest` is a **role class**, like `Platform` and `Sensor`, which this ADR's own addendum flags on the two adjacent rows and missed here |
| `Activity` | Observation acts, assignments, warning issuances | `prov:Activity` — **amended 2026-08-04 by ADR-006 Decision A.** This row also listed **hazard events**. The marker first given here was withdrawn the same day (BV7-1): it read *"`prov:Activity` is a provenance term for how a data artifact came to be, and a hazard produces none"*, which PROV-DM falsifies — an entity is *"a **physical**, digital, conceptual… thing"* and an activity *may* generate one. **A wildfire is a well-formed `prov:Activity`.** The ground is instead that this row's members are **acts performed in producing data and decisions**, and a hazard event is a **subject** those acts are about. What carries a hazard is Part 1 and is not decided |
