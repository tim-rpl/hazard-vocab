...
*17 of 21 cached graphs carry a provenance sidecar. Generated by `fetch-external.py`; edit the sidecars, not this block.*

**No sidecar, so no measured disposition — these are not covered by the table above:**

- `disaster-event-module-extensions.ttl`
- `disaster-event-module-generalized.ttl`
- `disaster-properties-ontology.ttl`
- `undrr-isc-hazard-classification.ttl`

They were supplied by hand because their namespaces could not be fetched here, so `fetch-external.py` has no measurement to write. **Listed rather than omitted:** a table that silently drops what it cannot measure reports coverage it does not have.
