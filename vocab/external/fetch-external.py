...
RB, RE = "<!-- BEGIN GENERATED:register -->", "<!-- END GENERATED:register -->"


def sync_register():
    """Generate README's register table from the sidecars.

    README.md says so itself: *if this table grows past a handful of
    rows, generate it from the provenance sidecars*. There are 21 graphs.
    A hand-maintained table keyed by name, beside the artifacts it
    describes, is the defect four gate rounds went to.

    **A graph with no sidecar is listed as a gap, not omitted.** Silent
    omission is how a table reports coverage it does not have.
    """
    import yaml as _y
    # F7: this enumerated `*.ttl` only, so a sidecar recording an
    # ATTEMPT THAT PRODUCED NO GRAPH was invisible and `0 gaps` could not
    # see it. `deo.provenance.yaml` carries `http_status "000"` and had no
    # `.ttl` beside it, so the register reported 21 rows / 0 gaps over
    # successes alone. **That is C11's own distinction — absent versus
    # zero — in this project's tooling.** Enumerate the union: a graph
    # with no sidecar is one gap, a sidecar with no graph is the other.
    rows, gaps, failed = [], [], []
    ...
    for g in sorted(CACHE.glob("*.ttl")):
        ...
        rows.append((g.stem, d.get("namespace", "-"),
                     ...
                     d.get("detail", "")))
    body = ["| Graph | Namespace | Dereferences | Disposition |",
            "|---|---|---|---|"]
    for k, ns, dr, disp, detail in rows:
        body.append("| `%s` | <%s> | **%s** | **%s** |" % (k, ns, dr, disp))
    body.append("")
    body.append("*%d of %d cached graphs carry a provenance sidecar, and "
                "%d fetch(es) produced no graph at all. Generated by "
                "`fetch-external.py`; edit the sidecars, not this block.*"
                % (len(rows), len(rows) + len(gaps), len(failed)))
    if failed:
        body += ["", "**Fetched and produced no graph.** A sidecar exists "
                 "and there is no `.ttl` beside it. Listed because an "
                 "attempt that failed is not the same as an attempt not "
                 "made, and a row count over successes cannot tell them "
                 "apart:", "",
                 "| Attempted | Source | HTTP |", "|---|---|---|"]
        body += ["| `%s` | <%s> | %s |" % f for f in failed]
    if gaps:
        body += ["", "**No sidecar, so no measured disposition — these are "
                 "not covered by the table above:**", ""]
        body += ["- `%s`" % g for g in gaps]
        body.append("")
        body.append("They were supplied by hand because their namespaces "
                    "could not be fetched here, so `fetch-external.py` has "
                    "no measurement to write. **Listed rather than omitted:** "
                    "a table that silently drops what it cannot measure "
                    "reports coverage it does not have.")
    text = README.read_text()
    if RB not in text or RE not in text:
        # F7's sibling, found the same round: this used to `return`
        # silently, so a README edited to drop the markers left the
        # generator a no-op that reported nothing. Two writers on one
        # file and the loser says nothing — the "inspected nothing" shape
        # in a generator rather than a check.
        print("FAIL  vocab/external/README.md has no "
              "%s / %s block, so the register was NOT generated. "
              "Restore the markers or say the table is hand-maintained; "
              "do not leave a generator that no-ops." % (RB, RE),
              file=sys.stderr)
        return 1
    i, j = text.index(RB), text.index(RE)
    README.write_text(text[:i] + RB + "\n\n" + "\n".join(body) + "\n\n" + text[j:])
    print("register: %d rows, %d gaps, %d failed fetch(es)"
