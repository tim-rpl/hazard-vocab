...
def sync_register():
    ...
    **A graph with no sidecar is listed as a gap, not omitted.** Silent
    ...
    body.append("")
    body.append("*%d of %d cached graphs carry a provenance sidecar. "
                "Generated by `fetch-external.py`; edit the sidecars, not "
                "this block.*" % (len(rows), len(rows) + len(gaps)))
    if gaps:
    ...
    print("register: %d rows, %d gaps" % (len(rows), len(gaps)))
