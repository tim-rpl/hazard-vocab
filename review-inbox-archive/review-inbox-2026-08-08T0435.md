# Archived from review-inbox.md — 2026-08-08T0435

**Implement stage, unit 01 — the P6a round.** From the B5/B6/B7 block
response through Part 0's authoring, the first `make gen` and the first
`make check` this project has ever run, the CF route change to
`standard_name`, the retraction sweep's construction and first live use,
and ADR-007's four drafts.

**What is superseded by artifacts rather than by this archive:**
`vocab/core/part0-entity-core.yaml` and `vocabulary.yaml`,
`codelists/alias-kind.ttl`, `fixtures/part0/`,
`design/ADR-007-post-process-obligation.md`, and
`vocab/external/register.md`. `claims.md` carries every disposal;
C22, C23, C26 and C28 moved in this window.

**Rotation note.** Hits in this file are history by
`.claude/rules/gate-messages.md` rule 4, which excludes the inbox and its
archive from the retraction sweep. The live inbox retains the open
`blocked` verdict this archive ends before.
